...
def handle_csrf_error(e):
    ...
    flash("Erro de segurança: token CSRF inválido. Por favor, tente novamente.")
    if request.path == '/processar_formulario':
        return redirect(url_for('formulario'))
    elif request.path == '/gerar_orcamento':
        return redirect(url_for('resumo_view'))
    elif request.path == '/enviar_orcamento':
...
# Adicionar log de requisições para depuração
@app.before_request
def log_request_info():
    """Log de informações da requisição para depuração"""
    if request.method == 'POST':
        app.logger.info(f"Requisição POST para {request.path}")
...
def gerar_orcamento():
    try:
        ...
        if not csrf_token:
            app.logger.error("Token CSRF ausente no formulário de geração de orçamento")
            flash("Erro de segurança: token CSRF ausente. Por favor, tente novamente.")
            return redirect(url_for('resumo_view'))
...
def processar_formulario():
    try:
        ...
        csrf_token = request.form.get('csrf_token')
        if not csrf_token:
            app.logger.error("Token CSRF ausente no formulário")
            flash("Erro de segurança: token CSRF ausente. Por favor, tente novamente.")
            return redirect(url_for('formulario'))
...
def enviar_orcamento():
    try:
        ...
        csrf_token = request.form.get('csrf_token')
        if not csrf_token:
            app.logger.error("Token CSRF ausente no formulário de envio de orçamento")
            flash("Erro de segurança: token CSRF ausente. Por favor, tente novamente.")
            return redirect(url_for('confirmacao'))
...
@csrf.exempt
def diagnostico():
    """Rota para diagnóstico do ambiente"""
    try:
        # Verificar ambiente
        from wsgi import verificar_ambiente
        ambiente_info = verificar_ambiente()
        
        diagnostico = {
            'ambiente': os.environ.get('FLASK_ENV', 'não definido'),
            'diretorio_atual': os.getcwd(),
            'arquivos_na_raiz': os.listdir(os.getcwd()),
            'flask_session_existe': os.path.exists(os.path.join(os.getcwd(), 'flask_session')),
            'precos_pgr_existe': os.path.exists(os.path.join(os.getcwd(), 'Precos_PGR.csv')),
            'precos_ambientais_existe': os.path.exists(os.path.join(os.getcwd(), 'Precos_Ambientais.csv')),
            'ambiente_info': ambiente_info,
            'python_version': sys.version,
            'platform': sys.platform,
            'variaveis_env': {
                'SECRET_KEY': os.environ.get('SECRET_KEY', 'não definido'),
                'FLASK_ENV': os.environ.get('FLASK_ENV', 'não definido'),
                'FLASK_APP': os.environ.get('FLASK_APP', 'não definido'),
                'EMAIL_REMETENTE': os.environ.get('EMAIL_REMETENTE', 'não definido'),
                'SESSION_TYPE': os.environ.get('SESSION_TYPE', 'não definido'),
                'SESSION_FILE_DIR': os.environ.get('SESSION_FILE_DIR', 'não definido')
            },
            'config': {
                'SECRET_KEY': app.config.get('SECRET_KEY', 'não definido'),
                'MAIL_SERVER': app.config.get('MAIL_SERVER', 'não definido'),
                'MAIL_PORT': app.config.get('MAIL_PORT', 'não definido'),
                'MAIL_USERNAME': app.config.get('MAIL_USERNAME', 'não definido'),
                'SESSION_TYPE': app.config.get('SESSION_TYPE', 'não definido'),
                'SESSION_FILE_DIR': app.config.get('SESSION_FILE_DIR', 'não definido')
            }
        }
        
        # Verificar se os arquivos CSV podem ser lidos
        try:
            if os.path.exists(os.path.join(os.getcwd(), 'Precos_PGR.csv')):
                df_pgr = pd.read_csv('Precos_PGR.csv')
                diagnostico['pgr_colunas'] = list(df_pgr.columns)
                diagnostico['pgr_num_linhas'] = len(df_pgr)
                diagnostico['pgr_primeiras_linhas'] = df_pgr.head(2).to_dict(orient='records')
        except Exception as e:
            diagnostico['erro_leitura_pgr'] = str(e)
            
        try:
            if os.path.exists(os.path.join(os.getcwd(), 'Precos_Ambientais.csv')):
                df_amb = pd.read_csv('Precos_Ambientais.csv')
                diagnostico['ambientais_colunas'] = list(df_amb.columns)
                diagnostico['ambientais_num_linhas'] = len(df_amb)
                diagnostico['ambientais_primeiras_linhas'] = df_amb.head(2).to_dict(orient='records')
        except Exception as e:
            diagnostico['erro_leitura_ambientais'] = str(e)
        
        # Verificar se o Flask-Mail está configurado corretamente
        try:
            from services.email_sender import init_mail
            mail_status = init_mail(app)
            diagnostico['mail_status'] = mail_status
        except Exception as e:
            diagnostico['mail_status_error'] = str(e)
        
        return jsonify(diagnostico)
    except Exception as e:
        app.logger.error(f"Erro na rota diagnóstico: {str(e)}")
